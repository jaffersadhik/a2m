...
public class ProcessMTM {
	...
	private static final String className = "[ProcessMTM]";
			...
			if (fileType.equalsIgnoreCase("txt")) {
				...
				while ((line = reader.readLine()) != null) {
					...
					if (stkTokens != null && stkTokens.length >= 2) {
						String msgType = stkTokens[0].trim();
						String mobileNo = stkTokens[1].trim();
						String message = "";
						int j = 2;
						for (int i = j; i < stkTokens.length; i++) {
							...
							if(StringUtils.isBlank(mobileNo)) {
								...
								if (newLineChar != null && newLineChar.trim().length() > 0) {
									if (msgType.equalsIgnoreCase(com.winnovature.utils.utils.Constants.MSG_TYPE_MULTI_LANG)) {
										message = UnicodeUtil.INSTANCE.convertHexStringToString(message);
									}
								...
								// platform will find dlt_template_id by it self.
								data.put("dlt_template_id", null);
						...
						//} // end of message not null
						//else {
							/*
							// prepare list - HO Invalid numbers to Q
							unProcessedRow = new UnprocessRow();
							unProcessedRow.setEsmeaddr(clientId);
							unProcessedRow.setReason(Constants.NO_MSG_FOR_DESTINATION);
							unProcessedRow.setFileId(fileId);
							unProcessedRow.setMobile(mobileNo);
							inValidList.add(unProcessedRow);
							failureCount++;
