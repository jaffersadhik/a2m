...
public class UnProcessListsCollector {
    ...
    public void unprocessHandoverToQueue(List<UnprocessRow> lstUnprocess) throws Exception{
        ...
        if (lstUnprocess != null && lstUnprocess.size() != 0) {
            try {
                ...
                log.error(className+methodName+" JSON processing Exception HO exclude numbers to redis", e);
                handleFallbackInsert(redisCon, lstUnprocess);
            } catch (Exception e) {
                log.error(className+methodName+" Exception HO exclude numbers to redis", e);
                handleFallbackInsert(redisCon, lstUnprocess);
            }
    ...
    private void handleFallbackInsert(Jedis redisCon, List<UnprocessRow> lstUnprocess) {
        try {
            if (redisCon != null) {
                redisCon.close();
            }
