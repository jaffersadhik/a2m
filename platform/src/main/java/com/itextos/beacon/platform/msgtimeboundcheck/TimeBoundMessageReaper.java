...
public class TimeBoundMessageReaper
        implements
        ITimedProcess
{

    private static final Log log             = LogFactory.getLog(TimeBoundMessageReaper.class);
    ...
    private static void checkForPreviousDate(
            int aRedisIndex)
    {
            ...
            int aRedisId)
    {
        ...
        try (
                Jedis lJedisCon = RedisConnectionProvider.getInstance().getConnection(ClusterType.COMMON, Component.TIMEBOUND_CHK, aRedisId);)
        {
            if (log.isDebugEnabled())
            ...
            {
                lPattern = TimeBoundConstants.REDIS_KEY_TIMECHECK_EXP + lPattern;
            ...
            else
            {
                ...
                lKeys        = lJedisCon.keys(TimeBoundConstants.REDIS_KEY_TIMECHECK_EXP + "*");
            }
            ...
            int aRedisIndex)
    {
        ...
        try
        {
            ...
            while (lFromTime.getTimeInMillis() < lEndDt)
            {
                ...
                try (
                        Jedis lRedisCon = RedisConnectionProvider.getInstance().getConnection(ClusterType.COMMON, Component.TIMEBOUND_CHK, aRedisIndex);)
                {
