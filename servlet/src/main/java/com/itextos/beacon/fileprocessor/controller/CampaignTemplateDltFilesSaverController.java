...
public class CampaignTemplateDltFilesSaverController {
    ...
    public Mono<Map<String, Object>> handleFileUpload(
        ...
        if (StringUtils.isBlank(ttype)) {
            return createErrorResponse(Constants.ERROR_CODE_REQUIRED_PARAMS_MISSING, 
                    "Bad Request", "t_type is required", HttpStatus.BAD_REQUEST);
        }
        
        if (StringUtils.isBlank(mobileColumn)) {
            return createErrorResponse(Constants.ERROR_CODE_REQUIRED_PARAMS_MISSING, 
                    "Bad Request", "mobile column/index is required", HttpStatus.BAD_REQUEST);
        }
        
        if (isInputInvalid(ttype, mobileColumn)) {
            return createErrorResponse(Constants.ERROR_CODE_REQUIRED_PARAMS_MISSING, 
                    "Bad Request", "Invalid mobile index " + mobileColumn, HttpStatus.BAD_REQUEST);
        }
    ...
    private boolean processPlaceholdersAndValidateMobile(Map<String, Object> result, String mobileColumnName, String ttype) {
        ...
        if (result.get("file_contents_column") != null) {
            List<List<String>> columns = (List<List<String>>) result.remove("file_contents_column");
            ...
            result.put("placeholders_column", headerAndDataPairFromFile);
            
            if ("column".equalsIgnoreCase(ttype)) {
        ...
        if (result.get("file_contents_index") != null) {
            List<List<String>> columns = (List<List<String>>) result.remove("file_contents_index");
            List<String> headers = columns.get(0);
            List<String> data = columns.get(1);
            Map<String, String> headerAndDataPairFromFile = new HashMap<>();
            ...
            result.put("placeholders_index", headerAndDataPairFromFile);
            
            if ("index".equalsIgnoreCase(ttype)) {
                headers.replaceAll(String::toLowerCase);
                if (!headers.contains(mobileColumnName)) {
                    mobileColumnFound = false;
                    result.put("message", "Missing Mobile Index " + mobileColumnName);
                }
            }
        }
    ...
    private boolean isInputInvalid(String ctype, String input) {
        if ("index".equalsIgnoreCase(ctype)) {
            try {
